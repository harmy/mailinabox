...
def mail_users():
	...
	if request.args.get("format", "") == "json":
		return json_response(get_mail_users_ex(env, limit=limit))
	else:
...
def mail_users_add():
	try:
		return add_mail_user(request.form.get('email', ''), request.form.get('password', ''), request.form.get('privileges', ''), env, request.form.get('forward_to', ''))
	except ValueError as e:
		return (str(e), 400)
